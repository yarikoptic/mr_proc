...
if [ "$#" -ne 14 ]; then
  ...
  echo "Sample cmd: ./heudiconv_stage_1.sh -d <dataset_root> -i <path_to_heudiconv_img> -r <singularity> -p <MNI01> -s <01> -l <./> -t 1"
  exit 1
fi
...
# if mr_proc dicoms are symlinks from some data store (e.g. scratch/raw_dicom)
